...
EmberAfStatus emberAfClusterSpecificCommandParse(EmberAfClusterCommand *cmd)
{
  ...
  if (cmd->direction == (uint8_t)ZCL_DIRECTION_SERVER_TO_CLIENT
    ...
    case ZCL_IDENTIFY_CLUSTER_ID:
      ...
      break;
    case ZCL_ENCELIUM_DIAGNOSTICS_CLUSTER_ID:
      result = emberAfEnceliumDiagnosticsClusterClientCommandParse(cmd);
      break;
    default:
    ...
    case ZCL_DIAGNOSTICS_CLUSTER_ID:
      result = status(false, true, cmd->mfgSpecific);
      break;
    case ZCL_ENCELIUM_INTERFACE_CLUSTER_ID:
      result = emberAfEnceliumInterfaceClusterServerCommandParse(cmd);
      break;
    case ZCL_ENCELIUM_DIAGNOSTICS_CLUSTER_ID:
      result = emberAfEnceliumDiagnosticsClusterServerCommandParse(cmd);
      break;
    case ZCL_ENCELIUM_EMERGENCY_CLUSTER_ID:
      result = emberAfEnceliumEmergencyClusterServerCommandParse(cmd);
      break;
    default:
      // Unrecognized cluster ID, error status will apply.
      break;
...
EmberAfStatus emberAfEnceliumInterfaceClusterServerCommandParse(EmberAfClusterCommand *cmd)
{
  bool wasHandled = false;
  if (cmd->mfgSpecific) {
    if (cmd->mfgCode == 0x110C && cmd->commandId == ZCL_RESCAN_INTERFACE_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t interfaceId;  // Ver.: always
      // Command is fixed length: 1
      if (cmd->bufLen < payloadOffset + 1u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
      interfaceId = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
      wasHandled = emberAfEnceliumInterfaceClusterRescanInterfaceCallback(interfaceId);
    } else if (cmd->mfgCode == 0x110C && cmd->commandId == ZCL_SET_INTERFACE_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t interfaceId;  // Ver.: always
      uint8_t interfaceType;  // Ver.: always
      // Command is fixed length: 2
      if (cmd->bufLen < payloadOffset + 2u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
      interfaceId = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
      payloadOffset += 1u;
      interfaceType = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
      wasHandled = emberAfEnceliumInterfaceClusterSetInterfaceCallback(interfaceId,
                                                                       interfaceType);
    }
  }
  return status(wasHandled, true, cmd->mfgSpecific);
...
EmberAfStatus emberAfEnceliumDiagnosticsClusterClientCommandParse(EmberAfClusterCommand *cmd)
{
  bool wasHandled = false;
  if (cmd->mfgSpecific) {
    if (cmd->mfgCode == 0x110C && cmd->commandId == ZCL_PROXIMITY_NODES_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t startIndex;  // Ver.: always
      uint8_t entries;  // Ver.: always
      uint8_t* proximityNodes;  // Ver.: always
      // Command is fixed length: 2
      if (cmd->bufLen < payloadOffset + 2u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
      startIndex = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
      payloadOffset += 1u;
      entries = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
      payloadOffset += 1u;
      proximityNodes = cmd->buffer + payloadOffset;
      wasHandled = emberAfEnceliumDiagnosticsClusterProximityNodesCallback(startIndex,
                                                                           entries,
                                                                           proximityNodes);
    }
  }
...
EmberAfStatus emberAfEnceliumDiagnosticsClusterServerCommandParse(EmberAfClusterCommand *cmd)
{
  bool wasHandled = false;
  if (cmd->mfgSpecific) {
    if (cmd->mfgCode == 0x110C && cmd->commandId == ZCL_REPORT_NEIGHBORS_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t startIndex;  // Ver.: always
      // Command is fixed length: 1
      if (cmd->bufLen < payloadOffset + 1u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
      startIndex = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
      wasHandled = emberAfEnceliumDiagnosticsClusterReportNeighborsCallback(startIndex);
    } else if (cmd->mfgCode == 0x110C && cmd->commandId == ZCL_REPORT_PROXIMITY_NODES_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t startIndex;  // Ver.: always
      // Command is fixed length: 1
      if (cmd->bufLen < payloadOffset + 1u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
      startIndex = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
      wasHandled = emberAfEnceliumDiagnosticsClusterReportProximityNodesCallback(startIndex);
    } else if (cmd->mfgCode == 0x110C && cmd->commandId == ZCL_RESET_PROXIMITY_NODES_DATA_COMMAND_ID) {
      // Command is fixed length: 0
      wasHandled = emberAfEnceliumDiagnosticsClusterResetProximityNodesDataCallback();
    } else if (cmd->mfgCode == 0x110C && cmd->commandId == ZCL_REPORT_AND_TRIGGER_PROXIMITY_NODES_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t startIndex;  // Ver.: always
      // Command is fixed length: 1
      if (cmd->bufLen < payloadOffset + 1u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
      startIndex = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
      wasHandled = emberAfEnceliumDiagnosticsClusterReportAndTriggerProximityNodesCallback(startIndex);
    }
  }
  return status(wasHandled, true, cmd->mfgSpecific);
}
...
EmberAfStatus emberAfEnceliumEmergencyClusterServerCommandParse(EmberAfClusterCommand *cmd)
{
  bool wasHandled = false;
  if (cmd->mfgSpecific) {
    if (cmd->mfgCode == 0x110C && cmd->commandId == ZCL_START_TEST_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t testType;  // Ver.: always
      // Command is fixed length: 1
      if (cmd->bufLen < payloadOffset + 1u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
      testType = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
      wasHandled = emberAfEnceliumEmergencyClusterStartTestCallback(testType);
    } else if (cmd->mfgCode == 0x110C && cmd->commandId == ZCL_STOP_TEST_COMMAND_ID) {
      // Command is fixed length: 0
      wasHandled = emberAfEnceliumEmergencyClusterStopTestCallback();
    } else if (cmd->mfgCode == 0x110C && cmd->commandId == ZCL_INHIBIT_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t enable;  // Ver.: always
      // Command is fixed length: 1
      if (cmd->bufLen < payloadOffset + 1u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
      enable = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
      wasHandled = emberAfEnceliumEmergencyClusterInhibitCallback(enable);
    } else if (cmd->mfgCode == 0x110C && cmd->commandId == ZCL_START_IDENTIFY_COMMAND_ID) {
      // Command is fixed length: 0
      wasHandled = emberAfEnceliumEmergencyClusterStartIdentifyCallback();
    } else if (cmd->mfgCode == 0x110C && cmd->commandId == ZCL_PROLONG_TIME_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t time;  // Ver.: always
      // Command is fixed length: 1
      if (cmd->bufLen < payloadOffset + 1u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
      time = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
      wasHandled = emberAfEnceliumEmergencyClusterProlongTimeCallback(time);
    }
  }
  return status(wasHandled, true, cmd->mfgSpecific);
}
